...
func main() {
	...
	if *crypt != "" && (*cph == "aes" || *cph == "anubis") && *mode == "GCM" {
		var keyHex string
		var keyRaw []byte
		if *pbkdf {
			keyRaw = pbkdf2.Key([]byte(*key), []byte(*salt), *iter, 32, myHash)
		...
		aead, err := cipher.NewGCMWithTagSize(ciph, 12)
		if err != nil {
			log.Fatal(err)
		}
	...
	if *crypt != "" && (*cph == "blowfish" || *cph == "idea" || *cph == "cast5" || *cph == "rc2" || *cph == "rc5" || *cph == "sm4" || *cph == "des" || *cph == "3des" || *cph == "seed") {
		var keyHex string
		keyHex = *key
		var key []byte
		var err error
